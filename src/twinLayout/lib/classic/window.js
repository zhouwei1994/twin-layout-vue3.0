...
export default function ClassicWindow(options, data, defaultPage) {
    ...
    this.$windowTopBarElem = $(`<div class="twin_classic_nav_bar_container_item twin_classic_nav_bar_container_item_active ${defaultPage ? 'twin_classic_default_page' : '' }"></div>`);
    ...
    if (!defaultPage) {
        this.$windowTopBarClose = $(`<i class="twin_classic_icon_garden_close"></i>`);
...
function centerWindow(options, dom){
    ...
    if (viewWidth < containerWidth) {
        ...
        if (offsetLeft < 0) {
            ...
            options.$navBarContainer.css("transform", "translateX(" + -difference + "px)");
        } else {
            options.navBarOffset = offsetLeft;
            options.$navBarContainer.css("transform", "translateX(" + -offsetLeft + "px)");
        }
    } else {
        options.navBarOffset = 0;
        options.$navBarContainer.css("transform", "translateX(0px)");
